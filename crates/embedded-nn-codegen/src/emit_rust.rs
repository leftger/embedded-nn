...
impl RustCodeGenerator {
    ...
    pub fn generate(&self, graph: &ModelGraph) -> String {
        ...
        if graph.inputs.len() == 1 && graph.outputs.len() == 1 {
            let in_shape = input_tensor.shape;
            let out_shape = output_tensor.shape;

            if in_shape.height == 1
                && in_shape.width == 1
                && out_shape.height == 1
                && out_shape.width == 1
            {
                let in_b = in_shape.batches;
                let in_cols = in_shape.channels;
                let out_b = out_shape.batches;
                let out_cols = out_shape.channels;

                out.push_str(&format!(
                    "    pub fn predict_tensor<'a>(\n        input: &Tensor2D<i8, {}, {}>,\n        arena: &'a mut [u8; ARENA_SIZE_BYTES],\n",
                    in_b, in_cols
                ));
                if has_svdf {
                    out.push_str("        svdf_state: &mut [i8; SVDF_STATE_BYTES],\n");
                ...
                out.push_str(&format!(
                    "    ) -> Result<Tensor2D<i8, {}, {}>, &'static str> {{\n",
                    out_b, out_cols
                ));
                out.push_str("        let mut out = Tensor2D::zero([Self::OUTPUT_SCALE], [Self::OUTPUT_ZERO_POINT]);\n");
                out.push_str("        let res_slice = Self::predict(input.as_slice(), arena");
                if has_svdf {
                    out.push_str(", svdf_state");
                }
                if has_lstm {
                ...
                out.push_str(")?;\n");
                out.push_str("        out.as_mut_slice().copy_from_slice(res_slice);\n");
                out.push_str("        Ok(out)\n");
                out.push_str("    }\n\n");
            } else {
                let (in_b, in_h, in_w, in_c) = (
                    in_shape.batches,
                    in_shape.height,
                    in_shape.width,
                    in_shape.channels,
                );
                let (out_b, out_h, out_w, out_c) = (
                    out_shape.batches,
                    out_shape.height,
                    out_shape.width,
                    out_shape.channels,
                ...
                out.push_str(&format!(
                    "    ) -> Result<Tensor4D<i8, {}, {}, {}, {}>, &'static str> {{\n",
                    out_b, out_h, out_w, out_c
                ));
                out.push_str("        let mut out = Tensor4D::zero([Self::OUTPUT_SCALE], [Self::OUTPUT_ZERO_POINT]);\n");
                out.push_str("        let res_slice = Self::predict(input.as_slice(), arena");
                if has_svdf {
                    out.push_str(", svdf_state");
                }
