...
pub use crate::model_zoo::ModelZooPreset;
...
impl StudioState {
    ...
    pub fn load_zoo_preset(&mut self, preset: ModelZooPreset) -> Result<(), String> {
        let title = preset.title().to_string();
        let graph = crate::model_zoo::build_preset(preset);
        self.install_imported_graph(graph, ModelSource::ZooPreset(title))
    }
