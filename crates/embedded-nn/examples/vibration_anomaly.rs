...
//! Run with:
//! ```console
//! cargo run --example vibration_anomaly --features="libm"
//! ```
...
fn main() {
    ...
    // Pre-trained autoencoder weights stored in Flash memory
    let flash_weights: [i8; 16] = [
        12, -4, 33, -8, 5, 20, -15, 7, -2, 18, -25, 14, 9, -11, 4, -30,
    ];

    // Calculate expected CRC at compile/build time
    let expected_crc = {
        let bytes = unsafe {
    ...
    // Boot check 1: Verify weights integrity against bitflips / Flash decay
    ...
    // Boot check 2: Verify SRAM activation arena capacity & canary protection
    let arena = [0u8; 128];
    // Place guard canary at the arena tail
    let canary = ARENA_GUARD_CANARY;
    match verify_arena_integrity(&arena, 64, canary) {
        Ok(()) => println!(
            "  -> [PASS] Arena bounds and canary verified (0x{:08X})",
            canary
        ),
    ...
    let autoencoder_detector = ReconstructionAnomalyDetector::new(50.0); // MSE threshold = 50.0

    // Test Case A: Normal motor vibration pattern
    let normal_vibration: [i8; 8] = [10, -12, 14, -8, 11, -10, 13, -9];
    let reconstructed_normal: [i8; 8] = [11, -11, 13, -9, 10, -11, 14, -8]; // Close reconstruction
    ...
    // Test Case B: Bearing Fault / Severe Vibration Spike
    let faulty_vibration: [i8; 8] = [10, -12, 85, -92, 110, -105, 95, -80]; // High amplitude spikes
    let reconstructed_faulty: [i8; 8] = [12, -10, 15, -8, 10, -12, 14, -9]; // Autoencoder fails to reconstruct unseen anomaly
    ...
    // Calibrated baseline distributions from healthy motor runs
    let baseline_mean = [1.25f32, 3.05f32, 4.10f32, 120.0f32];
    let baseline_inv_var = [1.0 / 0.05, 1.0 / 0.12, 1.0 / 0.20, 1.0 / 15.0]; // 1 / sigma^2
    let mahalanobis_detector =
        MahalanobisAnomalyDetector::new(&baseline_mean, &baseline_inv_var, 12.0);

    // Normal operating point
    let current_healthy = [1.28f32, 3.10f32, 4.22f32, 121.5f32];
    let score_healthy = mahalanobis_detector
        .score(&current_healthy)
        .expect("Score failed");
    ...
    // Unbalanced shaft operating point
    let current_unbalance = [2.85f32, 5.90f32, 9.80f32, 240.0f32];
    let score_unbalance = mahalanobis_detector
        .score(&current_unbalance)
        .expect("Score failed");
