...
fn main() {
    ...
    println!("  Packed Byte: 0x{:02X} (1 byte stores 2 weights)", packed as u8);
    println!("  Unpacked:   low = {:>2}, high = {:>2}", unpacked_low, unpacked_high);
    assert_eq!(val_low, unpacked_low);
    assert_eq!(val_high, unpacked_high);

    // 2. Executing a Quantized 4-Bit Fully Connected Layer (16 -> 8)
    ...
    let input_features: [i8; 16] = [
        12, -8, 25, 4, -18, 30, -5, 14,
        -22, 16, 9, -15, 27, -11, 8, -3,
    ];
    ...
    let codebook_lut: [f32; 16] = [
        -1.85, -1.20, -0.85, -0.55, -0.32, -0.18, -0.08, -0.01,
         0.01,  0.09,  0.21,  0.36,  0.60,  0.92,  1.35,  2.10,
    ];
